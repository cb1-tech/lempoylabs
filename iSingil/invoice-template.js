...
export function bankLines(b){return [b.bankName?b.bankName+(b.bankCode?` (${b.bankCode})`:''):b.nickname,b.branch?b.branch+(b.branchCode?` (${b.branchCode})`:''):'',[({Ordinary:'普通 / Ordinary',Current:'当座 / Current',Savings:'貯蓄 / Savings',Other:'その他 / Other'})[String(b.type||'').trim().replace(/^ordinary$/i,'Ordinary').replace(/^current$/i,'Current').replace(/^savings$/i,'Savings').replace(/^other$/i,'Other')]||b.type,b.number].filter(Boolean).join(' '),b.holder,b.currency?`Currency / 通貨: ${b.currency}`:'',b.swift?`SWIFT/BIC: ${b.swift}`:'',b.iban?`IBAN: ${b.iban}`:'',b.international,b.instructions].filter(Boolean)}
